...
export default function TeamPage() {
	return (
		<div className="bg-white">
			<div>
				<div className="h-[40rem] w-full flex md:items-center md:justify-center bg-white/[0.96] antialiased bg-grid-white/[0.02] relative overflow-hidden">
					...
					<div className="p-4 max-w-7xl mx-auto relative z-10 w-full pt-20 md:pt-0">
						<h1 className="text-3xl sm:text-4xl md:text-7xl font-bold text-center bg-clip-text text-transparent bg-gradient-to-b from-neutral-600 to-neutral-900 bg-opacity-50">
							Meet the Amazing Team
						</h1>
						<p className="mt-4 py-10 sm:py-20 font-normal text-neutral-600 max-w-2xl sm:max-w-5xl text-lg sm:text-2xl text-center mx-auto">
							Our team is a diverse group of passionate individuals dedicated to
							pushing the boundaries of technology and innovation. Together, we
							strive to create impactful solutions that make a difference in the
							world.
						</p>
					</div>
				</div>
			...
			<div>
				<h2 className="text-xl sm:text-2xl md:text-4xl font-bold text-center mt-8 mb-4">
					The Head
				</h2>
			</div>

			{/* Example responsive grid for team cards */}
			<div className="max-w-7xl mx-auto px-2 sm:px-4 py-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
				{/* {team.map((member) => (
          <Card key={member.name} className="flex flex-col items-center p-4">
            ...
            <CardContent className="text-center">
              <div className="text-sm mb-2">{member.description}</div>
              <div className="flex flex-wrap gap-2 justify-center">
                {member.expertise.map((exp) => (
                  <span key={exp} className="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs">{exp}</span>
                ))}
              </div>
            </CardContent>
          </Card>
